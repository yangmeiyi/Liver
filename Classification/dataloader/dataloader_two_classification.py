...
from LiLNet.transforms.my_transforms import AddGaussianNoise, AddSaltPepperNoise
...
class CancerSeT_CSV(Dataset):
    ...
    def __init__(self, root, type_):
        ...
        if type_ == "train":
            ...
            self.csv = self.root / "doctor.csv"  # BM_test.csv"
            self.transform_high = test_compose
            self.transform_low = test_compose
        elif type_ == "val_hn":
            self.csv = self.root / "BM_validation_hn.csv"  # "BM_validation_hn.csv"
            self.transform_high = test_compose
            self.transform_low = test_compose
        elif type_ == "val_cd":
