...
# Datasets
parser.add_argument('-d', '--dataset', default='FNH_HEM_CYST', type=str)
...
def save_acc_checkpoint(state, acc_is_best, checkpoint="./save_multi_model/2023_7_12/",
                        filename='resnet50_our.pth.tar'):
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if acc_is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'resnet50_our.pth.tar'))
...
def main():
    ...
    PATH = "./dataset_ct_ap_pvp_crop_224/"
    ...
    Liver_loader_val_sy = CancerSeT_CSV(PATH, 'val_sy')


    train_loader = torch.utils.data.DataLoader(Liver_loader_train, batch_size=args.train_batch, shuffle=True,
    ...
    sy_val_loader = torch.utils.data.DataLoader(Liver_loader_val_sy, batch_size=args.test_batch, shuffle=False)
    ...
    if up_load:
        checkpoint = torch.load(r"./save_multi_model/resnet50_our_fnh_hem_cyst_acc_1.pth.tar")
        model.load_state_dict(checkpoint['state_dict'])

    if pretrained:
        dict_trained = torch.load("./resnet50-19c8e357.pth")
    ...
    for epoch in range(start_epoch, args.epochs): #
        ...
        test_process(val_loader, model, criterion, use_cuda, args.beta, args.self_distillation)
        test_process(sy_val_loader, model, criterion, use_cuda, args.beta, args.self_distillation)
